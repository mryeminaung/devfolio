...
export default function TableOfContents() {
	...
	useEffect(() => {
		...
		const handleScroll = () => {
			...
			// Hide TOC when last heading is active
			const isLastActive =
				elements.length > 0 && currentId === elements[elements.length - 1].id;
			setIsVisible(!isLastActive);
		};
		...
		<AnimatePresence>
			{isVisible && (
				<motion.nav
					...
					className="hidden lg:block fixed top-46 w-60">
					<CornerAccent
						size={20}
						position="top-left"
						offset={10}
